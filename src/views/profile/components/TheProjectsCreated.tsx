...
import { useMultiCreateTokens } from '@/libs/sdk/hooks/useToken';
...
type TheProjectsCreatedProps = React.HTMLAttributes<HTMLElement> & {

}

const TheProjectsCreated = ({ ...attrs }: TheProjectsCreatedProps) => {
  ...
  const { data: tokenList, isLoading, refetch, isRefetching } = useMultiCreateTokens({
    endpoints,
    creator,
    variables: {
      first: pageSize,
  ...
  const { data: nextTokenList } = useMultiCreateTokens({
    endpoints,
    creator,
    variables: {
      first: pageSize,
...
export default TheProjectsCreated;
